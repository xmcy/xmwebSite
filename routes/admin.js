...
var captcha = require('../util/randomWord').captcha;
...
router.get('/vnum',function(req, res){
    var ary = captcha.get();
    req.session.vnum = ary[0].toLowerCase();
    res.end(ary[1]);
});
...
router.post('/manage/:defaultUrl/modify',function(req,res){
    ...
    }else if(targetObj == ContentType){
        ContentType.updateCategoryTemps(req,res,params.query.uid);
    }
...
router.get('/manage/articleCategorys/list', function(req, res) {
    return res.json(ContentType.find({}).sort({'sortId': 1}));
});
...
function addOneCategory(req,res){
    var errors;
    var newObj = new ContentType(req.body);
