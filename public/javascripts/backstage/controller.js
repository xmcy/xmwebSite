...
doraApp.controller("adminUserList",['$scope','$http','pageData','getItemService',function($scope,$http,pageData,getItemService){
    ...
    $('#addNewAdminUser').on('show.bs.modal', function (event) {
        ...
        // 如果不为空则为编辑状态
        if(editId){
            getItemService.itemInfo(pageData,editId).success(function(result){
                //console.log(result);
...
//文章类别管理
doraApp.controller('articleCategorys',['$scope','$http','pageData','getItemService', function ($scope,$http,pageData,getItemService) {
    // 初始化系统模板树
    initTreeDataByType($scope,$http,"tempTree");
    // 数据初始化
    $scope.warningInfo = {};
    ...
    getArticleCategoryData($scope,$http);
    //删除用户
    $('#checkIfDel').on('show.bs.modal', function (event) {
        var obj = $(event.relatedTarget);
        $scope.targetID = obj.data('whatever');
