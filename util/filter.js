...
function gen_session(user, res) {
    ...
    if(settings.debug){
        res.cookie(settings.auth_cookie_name, auth_token,
            {path: '/', maxAge: 1000 * 60 * 60 * 24 * 30, signed: true, httpOnly: true,domain:'.100ns.cn'}); //cookie 有效期30天
    }else {
        res.cookie(settings.auth_cookie_name, auth_token,
            {path: '/', maxAge: 1000 * 60 * 60 * 24 * 30, signed: true, httpOnly: true,domain:'.91zhizuo.com'}); //cookie 有效期30天
    }
...
function gen_sessionForOneDay(user, res) {
    var auth_token = user._id + '$$$$'; // 以后可能会存储更多信息，用 $$$$ 来分隔
    // TODO 正式上线后需要放开
    if(settings.debug){
        res.cookie(settings.auth_cookie_name, auth_token,
            {path: '/', expires:0, signed: true, httpOnly: true,domain:'.100ns.cn'});
    }else {
        res.cookie(settings.auth_cookie_name, auth_token,
            {path: '/', expires:0, signed: true, httpOnly: true,domain:'.91zhizuo.com'});
    }

    // res.cookie(settings.auth_cookie_name, auth_token,
    //         {path: '/', expires:0, signed: true, httpOnly: true});

}

exports.gen_session = gen_session;
exports.gen_sessionForOneDay = gen_sessionForOneDay;
