...
ContentTypeSchema.statics = {
    ...
    updateCategoryTemps : function(req,res,cateId){
        if(shortid.isValid(cateId)){
            ...
            ContentType.update(cateQuery,{$set:{contentTemp:req.body.contentTemp}},{multi : true},function(err){
                if(err){
                    res.end(err);
                }
            })
    ...
    getCateInfoById : function(cateId,callBack){
        ContentType.findOne({"_id": cateId}).populate('contentTemp').exec(function(err,doc){
            if(err){
                res.end(err);
            }else{
