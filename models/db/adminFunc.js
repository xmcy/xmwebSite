...
var adminFunc = {
  ...
  setBrandPromotionMngPageInfo: function (req, res, module) {
    var searchKey = ''
    var area = ''
    if (req.url) {
  ...
  setPageInfoforarticle: function (req, res, module) {
    var searchKey = ''
    // area是为了独立查询一个表其中的部分数据而设立的参数
    var area = ''
    if (req.url) {
    ...
    return {
      ...
      searchKey: searchKey,
      area: area,
      contentType:ContentType.find({}),
      editIs: editIs,
      currentLink: req.originalUrl,
      layout: 'manage/public/adminTemp'
    }
  },
  setPageInfoForAddEbookManage: function (req, res, module, traget) {
    var searchKey = ''
    // area是为了独立查询一个表其中的部分数据而设立的参数
    var area = ''
    if (req.url) {
    ...
    console.log(area)
    editIs = true
    Ebook.findOne({_id: req.query.uid}, function (err, doc) {
      if (err) {
        res.end(err)
      } else if (!_.isEmpty(doc)) {
        var addEbookManage = {}
        addEbookManage.doc = doc
        res.render(traget, {
  ...
  setPageInfoForAddVrManage: function (req, res, module, traget) {
    var searchKey = ''
    // area是为了独立查询一个表其中的部分数据而设立的参数
    var area = ''
    if (req.url) {
    ...
    console.log(area)
    editIs = true
    VR.findOne({_id: req.query.uid}, function (err, doc) {
      if (err) {
        res.end(err)
      } else if (!_.isEmpty(doc)) {
        var addVrManage = {}
        addVrManage = doc
        Company.findOne({_id: addVrManage.company}, function (err, company) {
          if (!company.companyName) {
            addVrManage.companyName = ''
          }else {
            addVrManage.companyName = company.companyName
          }
  ...
  setPageInfoForAddSlideManage: function (req, res, module, traget) {
    var searchKey = ''
    // area是为了独立查询一个表其中的部分数据而设立的参数
    var area = ''
    if (req.url) {
    ...
    console.log(area)
    editIs = true
    Slide.findOne({_id: req.query.uid}, function (err, doc) {
      if (err) {
        res.end(err)
      } else if (!_.isEmpty(doc)) {
        var addEbookManage = {}
  ...
  getTargetObj: function (currentPage) {
    ...
    }else if (currentPage === "articleManage_cateGory") {
